pipeline {
  ...
  environment {
    ...
    EXT_USER = 'popeen'
    EXT_REPO = 'Booksonic-Air'
    BUILD_VERSION_ARG = 'BOOKSONIC-AIR_RELEASE'
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-booksonic-air'
    CONTAINER_NAME = 'booksonic-air'
    DOCKERHUB_IMAGE = 'linuxserver/booksonic-air'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/booksonic-air'
    PR_DOCKERHUB_IMAGE = 'lspipepr/booksonic-air'
    DIST_IMAGE = 'alpine'
    MULTIARCH='true'
    CI='true'
    CI_WEB='true'
  ...
  stages {
    ...
    stage("Scarf.sh package registry"){
      ...
      steps{
        sh '''#! /bin/bash
              set -e
              PACKAGE_UUID=$(curl -X GET -H "Authorization: Bearer ${SCARF_TOKEN}" https://scarf.sh/api/v1/organizations/linuxserver-ci/packages | jq -r '.[] | select(.name=="linuxserver/booksonic-air") | .uuid')
              if [ -z "${PACKAGE_UUID}" ]; then
                echo "Adding package to Scarf.sh"
                curl -sX POST https://scarf.sh/api/v1/organizations/linuxserver-ci/packages \
                  -H "Authorization: Bearer ${SCARF_TOKEN}" \
                  -H "Content-Type: application/json" \
                  -d '{"name":"linuxserver/booksonic-air",\
                       "shortDescription":"example description",\
                       "libraryType":"docker",\
                       "website":"https://github.com/linuxserver/docker-booksonic-air",\
                       "backendUrl":"https://ghcr.io/linuxserver/booksonic-air",\
                       "publicUrl":"https://lscr.io/linuxserver/booksonic-air"}' || :
              else
